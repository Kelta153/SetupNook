...
async function runSpike(): Promise<Record<string, unknown>> {
  const raw =
    await prisma.$queryRaw`select now() as now, version() as pg_version`;

  const created = await prisma.spikeCheck.create({
    data: { note: `ENTRY.TS ran this — CLI entrypoint config test at ${new Date().toISOString()}` },
  });
...
Deno.serve(async (req: Request) => {
  ...
  if (url.pathname !== "/spike") {
    return new Response(
      "SetupNooK Prisma+Deno Deploy+Neon spike. GET /spike to run it.",
      {
        status: 200,
      },
    );
